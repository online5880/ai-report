...
def link_prediction_loss_hinge(g, embeddings):
    ...
    # Negative sampling with the same size as positive edges
    neg_src, neg_dst = dgl.sampling.global_uniform_negative_sampling(
        g, num_samples=len(pos_src)  # Match the number of positive edges
    )

    # Ensure the sizes of pos_scores and neg_scores match
